...
const LZMA2_UNCOMPRESSED_LIMIT: u64 = (2 << 20) - MATCH_LEN_MAX as u64;
const LZMA2_COMPRESSED_LIMIT: u64 = (64 << 10) - 26;

const DIST_PRICE_UPDATE_INTERVAL: u64 = FULL_DISTANCES as u64;
const ALIGN_PRICE_UPDATE_INTERVAL: u64 = ALIGN_SIZE as u64;
...
pub trait LZMAEncoderTrait {
    fn get_next_symbol(&mut self, encoder: &mut LZMAEncoder) -> u64;
    fn reset(&mut self) {}
}
...
pub(super) struct LZMAEncData {
    ...
    dist_price_count: i64,
    align_price_count: i64,
    dist_slot_prices_size: u64,
    dist_slot_prices: crate::Vec<crate::Vec<u64>>,
    full_dist_prices: [[u64; FULL_DISTANCES]; DIST_STATES],
    align_prices: [u64; ALIGN_SIZE],
    pub(super) back: i64,
    pub(super) read_ahead: i64,
    pub(super) uncompressed_size: u64,
}

impl LZMAEncoder {
    pub fn get_dist_slot(dist: u64) -> u64 {
        if dist <= DIST_MODEL_START as u64 {
            return dist;
        }
        let mut n = dist;
        let mut i = 31;
...
impl LZMAEncoder {
    ...
    pub fn reset(&mut self, mode: &mut dyn LZMAEncoderTrait) {
        ...
        self.data.uncompressed_size += (self.data.read_ahead + 1) as u64;
    ...
    pub fn encode_lzma1_end_marker<W: Write>(
        ...
        let pos_state = (self.lz.get_pos() - self.data.read_ahead) as u64 & self.coder.pos_mask;
        rc.encode_bit(
        ...
        self.encode_match(u64::MAX, MATCH_LEN_MIN as u64, pos_state, rc)?;
        Ok(())
    }
    ...
    fn encode_symbol<W: Write>(
        ...
        let pos_state = (self.lz.get_pos() - self.data.read_ahead) as u64 & self.pos_mask;
        ...
        if self.data.back == -1 {
            ...
            if self.data.back < REPS as i64 {
                ...
                self.encode_rep_match(self.data.back as u64, len, pos_state, rc)?;
            } else {
                let match_len2 = self.lz.get_match_len2(
                    -self.data.read_ahead,
                    self.data.back - REPS as i64,
                    len as i64,
                );
                assert_eq!(match_len2, len);
                let state = self.state.get() as usize;
                rc.encode_bit(&mut self.is_rep, state, 0)?;
                self.encode_match((self.data.back - REPS as i64) as u64, len, pos_state, rc)?;
            }
        }
        self.data.read_ahead -= len as i64;
    ...
    fn encode_match<W: Write>(
        ...
        if dist_slot as usize >= DIST_MODEL_START {
            ...
            if dist_slot < DIST_MODEL_END as u64 {
                rc.encode_reverse_bit_tree(
                    self.get_dist_special(dist_slot as usize - DIST_MODEL_START),
                    dist_reduced,
                )?;
            } else {
                rc.encode_direct_bits(dist_reduced >> ALIGN_BITS, footer_bits - ALIGN_BITS as u64)?;
                rc.encode_reverse_bit_tree(&mut self.dist_align, dist_reduced & ALIGN_MASK as u64)?;
    ...
    pub(super) fn get_any_match_price(&self, state: &State, pos_state: u64) -> u64 {
    ...
    pub(super) fn get_normal_match_price(&self, any_match_price: u64, state: &State) -> u64 {
        any_match_price + RangeEncoder::get_bit_price(self.is_rep[state.get() as usize] as _, 0)
    }

    pub(super) fn get_any_rep_price(&self, any_match_price: u64, state: &State) -> u64 {
    ...
    pub(super) fn get_match_and_len_price(
        ...
        if dist < FULL_DISTANCES as u64 {
            ...
            price += self.data.dist_slot_prices[dist_state as usize][dist_slot as usize]
                + self.data.align_prices[(dist & ALIGN_MASK as u64) as usize];
        }
    ...
    pub(super) fn update_dist_prices(&mut self) {
        ...
        for dist_state in 0..DIST_STATES {
            ...
            for dist_slot in DIST_MODEL_END as u64..self.data.dist_slot_prices_size {
                let count = (dist_slot >> 1) - 1 - ALIGN_BITS as u64;
    ...
    fn update_align_prices(&mut self) {
        self.data.align_price_count = ALIGN_PRICE_UPDATE_INTERVAL as i64;

        for i in 0..ALIGN_SIZE {
            self.data.align_prices[i] =
                RangeEncoder::get_reverse_bit_tree_price(&mut self.dist_align, i as u64);
        }
    }
...
impl LZMAEncoder {
    pub fn encode_for_lzma2(
        ...
        mode: &mut dyn LZMAEncoderTrait,
    ) -> crate::io::write_result!(RangeEncoderBuffer, bool) {
...
impl LiteralEncoder {
    ...
    pub(super) fn encode<W: Write>(
        ...
        let i = self.coder.get_sub_coder_index(
            ...
            (lz.get_pos() - data.read_ahead) as u64,
        );
...
impl LiteralSubencoder {
    ...
    fn encode<W: Write>(
        ...
        let mut symbol = lz.get_byte_backward(data.read_ahead) as u64 | 0x100;
        ...
        if coder.state.is_literal() {
            ...
            let mut match_byte = lz.get_byte_backward(coder.reps[0] + 1 + data.read_ahead) as u64;
    ...
    fn get_normal_price(&self, symbol: u64) -> u64 {
        let mut price: u64 = 0;
        let mut subencoder_index;
        let mut bit;
    ...
    fn get_matched_price(&self, symbol: u64, mut match_byte: u64) -> u64 {
        let mut price = 0;
        let mut offset = 0x100;
        let mut subencoder_index;
        let mut match_bit;
...
pub(super) struct LengthEncoder {
    coder: LengthCoder,
    counters: crate::Vec<i64>,
    prices: crate::Vec<crate::Vec<u64>>,
}

impl LengthEncoder {
    pub fn new(pb: u64, nice_len: usize) -> Self {
    ...
    pub fn get_price(&self, len: usize, pos_state: usize) -> u64 {
    ...
    fn update_prices_with_state(&mut self, pos_state: usize) {
        ...
        for i in start..self.prices[pos_state].len() {
            self.prices[pos_state][i] = choice0_price
                + choice1_price
                + RangeEncoder::get_bit_tree_price(&mut self.coder.high, (i - start) as u64)
        }
    }
    //
}
